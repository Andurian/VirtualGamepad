...
namespace VirtualGamepad
{
    class InputInterface
    {
        ...
        public static uint MapVKey(uint uCode, MappingType type)
        {
            return MapVirtualKey(uCode, (uint)type);
        }
        ...
        public static void MoveMouse(int dx, int dy)
        {
            ...
            input[0].type = 0;
            input[0].U.mi.mouseData = 0;
            input[0].U.mi.dx = dx;// *(65536 / GetSystemMetrics(SM_CXSCREEN));//x being coord in pixels
            input[0].U.mi.dy = dy;// *(65536 / GetSystemMetrics(SM_CYSCREEN));//y being coord in pixels
            input[0].U.mi.dwFlags = MOUSEEVENTF.MOVE;
            SendInput(1, input, Marshal.SizeOf(typeof(INPUT)));
        }
